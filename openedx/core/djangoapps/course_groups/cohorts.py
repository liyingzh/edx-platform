...
def add_user_to_cohort(cohort, username_or_email):
    ...
    Returns:
        User object, string (or None) indicating previous cohort, and whether the user is a preassigned user or not
    ...
    try:
        ...
        try:
            ...
            try:
                user = UnregisteredLearnerCohortAssignments.objects.get(
                    email=username_or_email, course_id=cohort.course_id
                )
                user.course_user_group = cohort
                user.save()
            except UnregisteredLearnerCohortAssignments.DoesNotExist:
                user = UnregisteredLearnerCohortAssignments.objects.create(
